...
export class OptimisticLockingSubscriber implements EntitySubscriberInterface {
  ...
  afterUpdate(event: UpdateEvent<never>) {
    if (event.metadata.versionColumn) {
      ...
      if (expectedVersion != actualVersion) {
        throw new OptimisticLockVersionMismatchError(event.entity, expectedVersion, actualVersion);
      }
    }
  }
}
